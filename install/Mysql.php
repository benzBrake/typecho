...
<?php elseif ('BAE' == $engine):
    ...
    <?php if (!getenv('HTTP_BAE_ENV_SK')):
        $baeDbPassword = "'{password}'"; ?>
        <li>
...
<?php elseif ('GAE' == $engine): ?>
    ...
    <?php if (0 === strpos($adapter, 'Pdo_')): ?>
        <li>
            ...
            <p class="description">
                <?php _e('请填入您在Cloud SQL面板中创建的数据库实例名称, 示例: %s', '<em class="warning">/cloudsql/typecho-gae:typecho</em>'); ?>
            </p>
        </li>
    <?php else: ?>
        <li>
